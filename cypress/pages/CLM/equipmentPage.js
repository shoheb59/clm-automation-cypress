...
export class equipMent {
  weblocators = {
    ...
    cb_SundayVerify:
      '[data-lang-key="APP_EQUIPMENTS.SUNDAY"] [type ="checkbox"]',
    txt_FROM: '[data-lang-key="APP_EQUIPMENTS.FROM"]',
    txt_TO: '[data-lang-key="APP_EQUIPMENTS.TO"]',
  };
  ...
  clickAddEquipment() {
    cy.get(this.weblocators.btn_addEquipment).click(
      { timeout: 3000 },
      { force: true }
    );
  }
  ...
  selectDropdownType() {
    ...
    cy.get(this.weblocators.dd_typeSelect)
      .contains("Forklift")
      .click({ force: true });
  }

  selectDropdownTypeRandomly() {
    cy.fixture("equipmentTypes").then((data) => {
      ...
      cy.get(this.weblocators.dd_typeSelect)
        .contains(randomItem)
        .click({ force: true });
      cy.log(`✅ Selected type: ${randomItem}`);
    });
  }
  ...
  selectSlotDuration15min() {
    ...
    cy.wait(2000);
    cy.get(this.weblocators.allSlotDuration)
      .contains("15")
      .should("be.visible")
      .click({ force: true });
  }
  ...
  selectAvailablilityMondayTOValue() {
    cy.get(this.weblocators.txt_TO)
      .should("be.visible")
      .then(() => {
        for (let i = 0; i < 2; i++) {
          ...
          cy.get(this.weblocators.btn_addIconHourList)
            .contains("add_circle")
            .click({ force: true });
        }
      });
