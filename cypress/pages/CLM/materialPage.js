export class material {
  ...
  navigateMaterial() {
    cy.get(this.weblocators.btn_materialManagement)
      .contains("Material Management")
      .click();
  }
  ...
  selectUnit() {
    ...
    cy.get(this.weblocators.dd_unit)
      .should("be.visible")
      .click({ setTimeout: 3000 }, { force: true });
    cy.wait(2000);
  ...
  collectMaterialName(callback) {
    cy.get(this.weblocators.row).then((rows) => {
      if (rows.length > 0) {
        cy.get(this.weblocators.first_column)
          .eq(1)
          .then(($column) => {
  ...
  deleteorKeepRow() {
    cy.get(this.weblocators.row)
      .eq(2)
      .find(this.weblocators.btn_delete)
      .click({ force: true });
    //Keep it
  ...
  createMaterial(materialData) {
    ...
    // Click Add Material
    this.clickCreateMaterial();

    // Material Name
    this.enterMaterialName(name);
    cy.wait(1000);
    // Select Unit (exact match)
    const safeUnit = unit.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // escape regex characters
    cy.get(this.weblocators.dd_unit).should("be.visible").click();
    cy.get(this.weblocators.dd_unitBundleOfPipes).contains(new RegExp(`^${safeUnit}$`, 'i')).click({ force: true });

    // Conditional logic for unit-based fields
    if (["EUR Pallet(s)", "Euro-mesh box", "Trolley"].includes(unit)) {
      cy.log(`ℹ️ Selected ${unit} — Length/Width are prefilled`);
      // No manual input needed
    } else if (["t", "m3"].includes(unit)) {
      cy.log(`ℹ️ Selected ${unit} — Length/Width fields not visible`);
      cy.get("body").then(($body) => {
        if ($body.find(this.weblocators.txt_length).length) {
          cy.get(this.weblocators.txt_length).should("not.be.visible");
        }
      });
    } else {
      cy.log(`ℹ️ Selected ${unit} — Entering custom length and width`);
      cy.get(this.weblocators.txt_length).clear().type(length);
      cy.get(this.weblocators.txt_width).clear().type(width);
    }
